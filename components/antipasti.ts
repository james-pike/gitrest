import type {PlaceItem} from "./place-list-item";

const antipasti: PlaceItem[] = [

  {
    id: "1",
    name: "Roasted Olives",
    
    href: "#",
    price: 11,
    description: "Marinated olives, fennel piquante, served with our Pinsa al forno bread.",
    imageSrc: "/images/roastedolives.jpg"
  },
  {
    id: "2",
    name: "Arancini Cheese",
   
    href: "#",
    price: 7,
    description: "Arborio risotto cooked in San Marzano tomato sauce, mozzarella and parmigano breaded and fried.",
    imageSrc: "/images/anti01.jpg"
  },

  {
    id: "3",
    name: "Arancini Mushrooms",

    href: "#",
    price: 8.00,
    description: "Arborio risotto, mushrooms, white wine, parmigano, breaded and fried.",
    imageSrc: "/images/anti02.jpg"
  },

  {
    id: "4",
    name: "Salumi Board",

    href: "#",
    price: 32.00,
    description: "Italian meats & cheeses, oven roasted olives and vegetables served with our pinsa al forno bread.",
    imageSrc: "/images/salumi.jpg"
  },
  {
    id: "5",
    name: "Bruschetta",
   
    href: "#",
    price: 15,
    description: "Roma Tomatoes, Grana Padano, Olive Oil, Basil, Balsamic Glaze, served on our Pinsa al Forno Bread.",
    imageSrc: "/images/bruschetta.jpg"
  },

 

  {
    id: "5",
    name: "Polpette",
   
    href: "#",
    price: 18,
    description: "Italian Style veal, pork and beef meatballs topped with Tomato Sauce, Whipped Ricotta, Grana Padano, served with a side of our Pinsa al Forno Bread.",
    imageSrc: "/images/polpette.jpg"
  },
  {
    id: "6",
    name: "Tuscan Wings",
   
    href: "#",
    price: 19,
    description: "Herbed crispy Chicken Wings, Served with a side of Bomba Aioli.",
    imageSrc: "/images/anti1.jpg"
  },
  {
    id: "7",
    name: "Al Forno Bread",
   
    href: "#",
    price: 7,
    description: "Pinsa Al Forno Bread Served with Extra Virgin Olive Oil & Balsamic Vinaigrette.",
    imageSrc: "/images/alforno.jpg"
  },


];

export default antipasti;
